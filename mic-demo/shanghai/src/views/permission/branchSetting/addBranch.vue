<template>
    <Card dis-hover>
        <div class="h4-title">
            <span class="div-title-span">新增分店</span>
        </div>
        <Form ref="form" 
            :model="formData"
        ...
        >
            ...
            <Row>
                <Col span="24">
                    <FormItem label="门店照片" class="ov-form-item ivu-form-item-required" prop="pic">
                        <imageUpload 
                            :uploadButton="'upload'" 
							:maxUploads="1" 
                            :isPhone="true"
...
export default {
    ...
    mounted () {
        if(this.$store.state.userInfo.storeInfo){
            this.formData.parentId=this.$store.state.userInfo.storeInfo.id;
        }else{
            this.getUserInfo();
        }
        this.getCode();
