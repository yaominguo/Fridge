	...
	<div>
		<Card class="card house-card" dis-hover>
			...
			<HouseCardList v-show="canShowCenterView" v-for="(community,index) in houseList" :key="index" :buildType="1" :list="community.list"
			 :header="community.name" :buildingCode="community.buildingCode" :subTitle="community.address" :isCentral="true">
				<div slot="functionMenu" class="house-card-list-header-function">
					<span @click="massAddHouse(community)">批量添加房间</span>
					<router-link :to="'/house/multiRoomManagement/'+community.id">管理房间</router-link>
				</div>
			</HouseCardList>
...
<style scoped>
	...
	.house-card-list-header-function span {
		color: #2d8cf0;
		cursor: pointer;
	}
