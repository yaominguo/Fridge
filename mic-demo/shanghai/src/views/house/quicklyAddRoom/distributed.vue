<template>
	<div>
		<Card class="card" dis-hover :bordered="false">
			<!-- 步骤条 -->
			<div id="steps" v-if="!quickAdd&&quickAdd!=1">
				...
				<Form ref="formInline" :model="createHouse" :rules="ruleCreateHouse" :label-width="80" id="createForm">
					...
					<Row type="flex" justify="start" :gutter="16">
						...
						<FormItem label="楼栋" prop="buildingId">
							<Select v-model="createHouse.buildingId" placeholder="请选择" @on-change="buildingChange" :disabled="current=='1'||quickAdd=='1'"
							 transfer>
								 ...
								 v-if="count.length>0" :rules="formDynamicSecondRules">
									...
									<div class="middle">
										<FormItem :prop="'roomSpace'">
											<Input type="text" v-model="item.roomSpace" placeholder="房间面积"></Input>
										</FormItem>
									</div>
									<div class="large" style="padding: 8px 0 0 8px!important;">
										<FormItem :prop="'payNum'" style="display:inline-block;margin-right:5px">
											<Select v-model="item.payNum" transfer style="width:65px" placeholder="付">
										...
										<FormItem :prop="'mortgageNum'" style="display:inline-block;margin:0">
											<Select v-model="item.mortgageNum" style="width:65px" placeholder="押" transfer @on-change="mortgageNumChange(item.mortgageNum,it,index)">
									 ...
									 :rules="formDynamicSecondRules">
										<div style="width:100%;border:1px solid #e8eaec;margin-bottom:10px" @click="changeActiveItems('2',index)"
										 :class="{'houseBlockActive':houseAllActiveIndex==index}">
											<div class="large" style="padding:17px 0 0 0;text-align:center">
											...
											<div class="middle">
												<FormItem :prop="'houseSpace'">
													<Input disabled type="text" v-model="item.houseSpace" placeholder="房屋面积"></Input>
												</FormItem>
											</div>
											<div class="large" style="padding: 8px 0 0 12px!important;">
												<FormItem :prop="'payNum'" style="display:inline-block;margin-right:5px">
													<Select v-model="item.payNum" style="width:60px" placeholder="付" transfer>
														<Option v-for="count in 12" :value="String(count)" :key="count">{{ count }}</Option>
													</Select>
												</FormItem>
												<FormItem :prop="'mortgageNum'" style="display:inline-block;margin:0">
													<Select v-model="item.mortgageNum" style="width:60px" placeholder="押" transfer @on-change="mortgageNumChange(item.mortgageNum,index,'')">
														<Option value="auto">自定义</Option>
														<Option v-for="count in 12" :value="String(count)" :key="count">{{ count }}</Option>
													</Select>
												</FormItem>
			...
			<div class="stepNext" v-if="current==0">
				<Button @click="returnDistributeAdd()" v-if="quickAdd">上一步</Button>
			...
			<Modal v-model="modal" title="设置" @on-ok="ok" @on-cancel="cancel" width="500">
				...
				<Row type="flex" justify="center" align="middle" v-if="modalType=='area'">
					<Col span="8">
					将列表中<span class="required">建筑面积</span>统一设置为:
					</Col>
					<Col span="12">
				...
				<Row type="flex" justify="center" align="middle" v-if="modalType=='publicSet'">
					...
					<Col span="12" v-if="houseSetSingleAll!==''">
					<CheckboxGroup v-model="publicSets">
						<Row type="flex" justify="space-between" v-for="(item,index) in  configLists" :key="index" v-if="index==houseSetSingleAll">
	...
	export default {
		...
		methods: {
						...
						if (this.communityList.length > 0) {
							this.communityList.forEach(item => {
								if (this.quickAdd && this.quickAdd == "1") {
									if (item.id == this.transferData.communityId) {
										this.buildingLists = item.buildingList;
										this.createHouse.communityId = this.transferData.communityId;
										this.createHouse.buildingId = this.transferData.buildingId;
										this.buildingName = this.transferData.buildingName;
										this.createHouse.floorCount = this.transferData.floorCount;
										this.createHouse.elevator = this.transferData.isElevator;
										this.showIsDisabled();
									}
								} else {
									if (item.id == this.$route.query.id) {
										this.buildingLists = item.buildingList;
										this.createHouse.communityId = this.$route.query.id;
									}
								}
							})
						}
					...
					this.$api.HOUSE_CONFIG, {}, (res) => {
						...
						data.forEach((e, index) => {
							if (e.isDefault == "01") {
								...
								let array = [];
								e.configResources.forEach(ele => {
									if (ele.isChecked == '01') {
										array.push(ele.name);
									}
								})
								this.publicSets = array;
							}
						});
					}
				)
			...
			buildingChange() {
				this.buildingLists.forEach(item => {
					if (item.id == this.createHouse.buildingId) {
						...
						this.createHouse.elevator = item.isElevator != null ? String(item.isElevator) : "";
						this.showIsDisabled("query");
						this.$refs.formInline.validateField('elevator');
						this.$refs.formInline.validateField('floorCount');
					}
				})
			},
			showIsDisabled(query) {
				let str = "";
				if (!query) {
					str = String(this.transferData.isElevator);
				} else {
					str = String(this.createHouse.elevator);
				}
				...
				if (flag) {
					if (this.createHouse.leaseType == 1) {
						...
						this.formDynamic.items.forEach((item, index) => {
							...
							this.publicEuipment["item" + index] = this.publicSets;
								...
								res => {
									...
									for (let i = 0; i < item.roomNum; i++) {
										...
										if (this.houseSetSingleAll !== "") {
											...
											obj.roomConfigList = this.publicSets;
										}
										this.$forceUpdate();
						...
						this.formDynamic.items.forEach((item, index) => {
							...
							if (this.houseSetSingleAll !== "") {
								...
								item.roomConfigList = this.publicSets;
							}
							this.$ajax.get(this, this.$api.GET_FILE_UPLOAD_CODE_GET, {}, res => {
								this.codeArray += res.data.content.code + ",";
			...
			ok() {
				...
				this.unite[this.modalType] = this.unite[this.modalType] && (this.modalType == "roomPicList" || this.modalType ==
					"housePicList") ? [] : "";
				this.unite.roomCount = "";
				let validText = this.current == "0" ? 'formDynamic' : 'formDynamicSecond';
				let validItem = this.modalType;
				let validMessage = "";
				switch (validItem) {
					case "house":
						validMessage = "roomNum";
						break;
					case "hall":
						validMessage = "parlourNum";
						break;
					case "guard":
						validMessage = "toiletNum";
						break;
					case "floorAll":
						validMessage = "floorCount";
						break;
					case "area":
						validMessage = "houseSpace";
						break;
					case "methods":
						validMessage = "payNum-mortgageNum";
						break;
					default:
						validMessage = validItem;
						break;
				}
				this.$refs[validText].forEach(item => {
					if (validMessage.indexOf('-') != -1) {
						let array = validMessage.split('-');
						item.validateField(array[0]);
						item.validateField(array[1]);
					} else {
						item.validateField(validMessage);
					}
				});
			},
			...
			cancel() {
				this.modal = false;
				this.unite.roomCount = "";
				this.unite[this.modalType] = this.modalType == "roomPicList" || this.modalType == "housePicList" ? [] : "";
		...
		watch: {
			...
			"unite.roomConfigList"() {
				if (this.unite.houseSetSingle !== "" && this.unite.houseSetSingle != undefined && this.configLists && this.configLists
					.length > 0) {
					let str = this.unite.roomConfigList && this.unite.roomConfigList.length > 0 ? this.unite.roomConfigList.join(",") :
						'';
					let array = [];
					let str2 = "";
					this.configLists[this.unite.houseSetSingle].configResources.forEach(ele => {
