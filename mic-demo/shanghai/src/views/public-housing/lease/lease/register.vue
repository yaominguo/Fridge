  ...
  data() {
    ...
    return {
      ...
      endOptions:{
        ...
        disabledDate :function(date) {
          //结束时间不小当前时间且不小于开始时间
            return  date.valueOf() < new Date() - 86400000 && date.valueOf() < this.leaseModel.startTime.valueOf()-86400000;
        }.bind(this)
      },
      title:'租客登记',
      showAddress:false,
    ...
    getRoomDetail(roomId){
      this.$ajax.get(
          ...
          res => {
            ...
            //公租房特有 查询配租单填入信息或置灰提交按钮
            this.selectTenant(info.no?info.no:'0000');
            ...
            //公共事业缴费 有无户号
              if(!info.houseNum){//无户号，禁用表单&默认房东缴费
        ...
        if(item && item.label){
          ...
          this.houseOptions.forEach((child)=>{
            ...
            if(child.value == item.value){
              ...
              this.selectTenant(child.no?child.no:'0000');
      ...
      this.planList.forEach((item,index)=>{
        if(index == 0){//第一个月 一次性费用在里面
          item.onceFeeList.forEach( child =>{
              ...
              temObj.number = 1;
              temObj.receivableTime = item.dateEnd;
              temObj.status = '0';
