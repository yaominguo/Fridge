          ...
          <Row>
            <Col span="8">
              <Select v-model="formModel.templateId">
                <Option v-for="(option, index) in contractModelList" :value="option.id" :key="index">{{option.templateName}}</Option>
              </Select>
            </Col>
            <Col span="6" style="textAlign: center">
              <Button to="#/contractTemplate/add" target="_blank" type="primary" ghost icon="md-add">点击创建新电子合同模板</Button>
            </Col>
          </Row>
        </FormItem>
      </Form> -->
...
export default {
  ...
  data() {
    ...
    return {
      ...
      leaseModel: {
        ...
        collectType: '1', // 收租方式 0-提前收租 1-每期的第一个月 2-每期提前一个月
        payNum: 1,
        beforeDay:10,
        fixedFirst:1,
        fixedEarly:1,
  ...
  methods: {
    ...
    appendTenantAndDisabled(info){
      //填入数据
      this.$set(this.tenantModel,'phone',info.phone);
      this.$set(this.tenantModel,'name',info.tenantName);
      this.$set(this.tenantModel,'certNo',info.idCard);
      //表单置灰
      this.tenantLayout[0].phone.disabled = true;
      this.tenantLayout[0].name.disabled = true;
      // this.tenantLayout[0].isLesseeOrigin.options[0].disabled = true;
