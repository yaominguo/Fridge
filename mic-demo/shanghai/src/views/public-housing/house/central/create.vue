			...
			<Form ref="formInline" :model="createHouse" :rules="ruleCreateHouse" :label-width="100" class="createForm">
				...
				<div class="containerFlow">
					<FormItem label="公寓名称">
						<Row>
							<Col span="13">
							<Input v-model="$route.params.name" disabled="disabled" />
							</Col>
						</Row>
					</FormItem>
					<!-- 是否电梯和楼层总高 -->
						...
						<Form ref="floorInline" :model="item" :rules="ruleFloorInline" :label-width="100" class="createForm" style="overflow: hidden;padding:0;padding-top:8px;">
							...
							<FormItem label="所在楼层" prop="floor">
								<Select transfer v-model="item.floor" clearable placeholder="请选择" @on-change="personalSetRoomCode(index,item.floor)">
									<Option v-for="count in Number($route.params.floorCount)" :value="String(count)" :key="count">{{ count }}</Option>
								</Select>
							</FormItem>
							</Col>
...
<script>
	export default {
		...
		computed: {
			isDisabledEle() {
				if (this.$route.params.elevator == undefined || this.$route.params.elevator == null || this.$route.params.elevator ==
					'') {
					return false;
				} else {
					this.createHouse.elevator = this.$route.params.elevator == "true" ? "1" : "0";
					return true;
				}
			},
			roomObj() {
		...
		methods: {
			getCommunity() {
				...
				this.createHouse.floorCount = this.$route.params.floorCount;
				this.createHouse.buildingId = this.$route.params.id;
				this.createHouse.communityId = this.$route.params.communityId;
