<template>
	<Card>
		<div class="editTitle">
			{{detail.showName}}
			<!-- <span v-if="pageType=='disperseSingleRoom'">({{detail.alias}})</span> -->
			 ...
			 v-if="pageType=='editRoommatesEntire'">
				...
				<Row>
					...
					<FormItem label="公共区域配置" :label-width="90">
						<Radio v-model="publicSetSingle" style="margin-bottom:10px">标配</Radio>
						<CheckboxGroup v-model="publicSets">
							...
							<Checkbox label="燃气灶"></Checkbox>
							<Checkbox label="电磁炉"></Checkbox>
							<Checkbox label="沙发"></Checkbox>
							<Checkbox label="抽油烟机"></Checkbox>
						</CheckboxGroup>
					</FormItem>
					</Col>
				</Row>
				...
				<Row>
					<FormItem label="收租日设置">
						<RadioGroup v-model="rentType" vertical @on-change="rentTypeChange">
							<Radio label="0" style="margin-bottom:10px">
								<span>提前</span>
								<Input type="text" v-model="dayAdvance" :disabled="rentTypeDisabled!=0" class="itemWidth" @on-blur="formatDay(dayAdvance,'dayAdvance')"></Input>
								<span>天收租</span>
							</Radio>
							<Radio label="1" style="margin-bottom:10px">
								<span>固定</span>
								<Input type="text" v-model="dayStable" :disabled="rentTypeDisabled!=1" class="itemWidth" @on-blur="formatDay(dayStable,'dayStable')"></Input>
								<span>日收租 (每期的第一个月)</span>
							</Radio>
							<Radio label="2">
								<span>固定</span>
								<Input type="text" v-model="dayReStable" :disabled="rentTypeDisabled!=2" class="itemWidth" @on-blur="formatDay(dayReStable,'dayReStable')"></Input>
					...
					<FormItem label="房间配置">
						<CheckboxGroup v-model="roomConfigList">
							...
							<Row type="flex" justify="start">
								...
								<Col>
								<Checkbox label="书桌"></Checkbox>
								</Col>
								<Col>
								<Checkbox label="衣柜"></Checkbox>
								</Col>
								<Col>
								<Checkbox label="阳台"></Checkbox>
								</Col>
								<Col>
								<Checkbox label="独卫"></Checkbox>
								</Col>
							</Row>
						</CheckboxGroup>
					</FormItem>
				...
				<Row>
					<FormItem label="房间配置">
						<RadioGroup v-model="unite.houseSetSingle" style="margin-bottom:10px">
							<Radio label="1">主卧</Radio>
							<Radio label="2">次卧</Radio>
						</RadioGroup>
		...
		<Modal v-model="editNumber" title="编辑门牌号" v-if="editNumber">
			<Form ref="editRoomCode" :model="editRoomCode" :label-width="80" :rules="editRoomCodeRules">
			...
			<div slot="footer">
				...
				<Button type="default" @click="cancel('editRoomCode')">取消</Button>
			</div>
		</Modal>
	</Card>
</template>
	...
	export default {
		...
		methods: {
			...
			classifyData(res) {
				...
				let array = [];
				let arrayHouse = [];
				// 				this.provinceCommunity = datas.provName + datas.name;
				// 				this.showName = datas.showName;
				// 				this.buildingCode = datas.buildingCode;
				...
				if (this.pageType == "editSingleRoomListings") {
					datasets = {
						...
						rentType: datas["rentType"] != null ? datas["rentType"] : this.rentType,
						rentDay: datas["rentDay"] != null ? datas["rentDay"] : this.dayAdvance,
				...
				switch (rentType) {
					case "0":
						this.dayAdvance = renDay;
						break;
					case "1":
						this.dayStable = renDay;
						break;
					case "2":
						this.dayReStable = renDay;
						break;
					default:
						break;
				}
			...
			showModal() {
				if (this.isUpdateHouse == true) {
					...
					this.editRoomCode.buildingCode = this.buildingCode;
					...
					res => {
						if (query == 1) {
							...
							this.roomCostList.forEach((element, index) => {
								...
								this.gainfeeChildrenList(element, index, "haha");
							});
						} else {
							this.$forceUpdate();
							this.roomCostList[index].secondsLists = res.data.content;
						}
					}
				);
			},
