...
export default {
          ...
          render: (h, params) => {
            ...
            return h('div', [
              h('Button', {
                ...
                on: {
                  click: () => this.$router.push({name: 'room-detail', query: {id: params.row.id}})
                },
              } ,'查看'),
              btn(),
            ])
  ...
  methods: {
    getOptions() {
      const { ASSET_COMMUNITY } = this.$api;
      const list = [
      ...
      return list.map(item =>
        this.$ajax.get(
          ...
          (res) => {
            ...
            this.searchLayout[0].communityId.options.push(...list);
          },
          true,
        )
      );
    ...
    handleAudit() {
      this.$refs.form.validate(valid => {
        ...
        this.$ajax.put(
          this,
          this.$api.POST_BACK_VERIFY.replace('{id}', this.curId) + '?status=' + status + '&reason=' + this.auditInfo.reason,
          null,
          () => {
