...
<template>
  <Card dis-hover>
    <h1 style="text-align: center;margin: 20px 0;">修改租约</h1>
    ...
    <Card dis-hover title="租约信息">
      <Form ref="leaseForm" label-position="right" :label-width="140" :model="leaseModel" :rules="rules">
        <Row>
              ...
              <Row>
                  <Col span="11">
                      <FormItem prop="startTime">
                          <DatePicker :disabled="isEdit" type="date" :options="startOptions" placeholder="开始日期" v-model="leaseModel.startTime" @on-change="canShowPlan"></DatePicker>
                      </FormItem>
                  </Col>
                  <Col span="2" style="text-align: center">至</Col>
                  <Col span="11">
                      <FormItem prop="endTime">
                          <DatePicker :disabled="isEdit" type="date" :options="endOptions" placeholder="截止日期" v-model="leaseModel.endTime" @on-change="canShowPlan"></DatePicker>
                      </FormItem>
                  </Col>
              </Row>
            </FormItem>
          ...
          <Col span="3">
            <FormItem :label-width="50" class="item-label" label="付" prop="payNum">
              <Select :disabled="isEdit" style="width: 80px; fontWeight: normal;" v-model="leaseModel.payNum" @on-change="canShowPlan">
          ...
          <Col span="3">
            <FormItem :label-width="50" class="item-label" label="押" prop="mortgageNum">
              <Select :disabled="isEdit" style="width: 80px; fontWeight: normal;" v-model="leaseModel.mortgageNum" @on-change="changeLeaseCost('mortgageNum')">
        ...
        <Row>
          <Col span="8">
            <FormItem class="item-label" label="房屋租金（元/月）" prop="rent">
              <Input :disabled="isEdit" placeholder="请输入金额（元/月）" v-model="leaseModel.rent" @on-change="changeLeaseCost" number />
            </FormItem>
          </Col>
          <Col span="8">
            <FormItem class="item-label" label="房屋押金（元）" prop="deposit">
              <Input :disabled="isEdit" placeholder="请输入金额（元）" v-model="leaseModel.deposit" @on-change="changeLeaseCost('deposit')" number />
            </FormItem>
          </Col>
        </Row>
        <Row>
          ...
          <Col span="8">
            <FormItem class="item-label" label="公共事业缴费" prop="landlordCharges">
              <RadioGroup   v-model="leaseModel.landlordCharges" @on-change="landlordChange">
                  <Radio :disabled="landLoadDisabled || isEdit" label="1" >
                    房东
                  </Radio>
                  <Radio :disabled="landLoadDisabled || isEdit" label="0">
                    租客
                  </Radio>
              </RadioGroup>
            </FormItem>
        ...
        <FormItem class="item-label" label="其他费用">
          <Row v-for="(item,index) in feeList" :key="index" style="marginBottom: 10px;">
            <Col span="5">
              <FormItem>
                <Cascader :disabled="isEdit" v-model="item.name" :clearable="false" @on-change="feeTypeChange" :data="feeTypeList" trigger="hover"/>
              </FormItem>
            </Col>
            <Col offset="1" span="3">
              <FormItem >
                <InputNumber :disabled="isEdit"  v-model="item.count" :min="0" @on-change="canShowPlan" type="text" placeholder="请输入金额" />
              </FormItem>
            </Col>
            <Col span="5">
              <FormItem>
                <Input :disabled="isEdit" v-model="item.ps" type="text" placeholder="请输入费用备注"  />
              </FormItem>
            </Col>
            <Col span="3">
              <Button :disabled="isEdit" v-if="feeList.length >= 1" @click="deleteRow(index)" class="delete-row" shape="circle" icon="md-close"></Button>
            </Col>
          </Row>
        </FormItem>
       <!--  <FormItem>
        ...
        <FormItem class="item-label" label="收租日期" prop="collectType">
          <RadioGroup @on-change="canShowPlan" vertical style="fontWeight:normal" v-model="leaseModel.collectType">
            <Radio :disabled="isEdit" label="1" style="margin-bottom:24px">
              <span>提前</span>
              <FormItem prop="beforeDay" style="display: inline-block;">
                <InputNumber :disabled="leaseModel.collectType != '1' || isEdit" v-model="leaseModel.beforeDay" @on-change="canShowPlan" :max="99" :min="0"/>
              </FormItem>
              <span>天收租</span>
            </Radio>
            <Radio :disabled="isEdit" label="2" style="margin-bottom:24px">
              <span>固定</span>
              <FormItem prop="fixedFirst" style="display: inline-block;">
              <InputNumber :disabled="leaseModel.collectType != '2' || isEdit" v-model="leaseModel.fixedFirst" @on-change="canShowPlan" :max="31" :min="1" />
              </FormItem>
              <span>日收租 (每期的第一个月)</span>
            </Radio>
            <Radio :disabled="isEdit" label="3">
              <span>固定</span>
              <FormItem prop="fixedEarly" style="display: inline-block;">
              <InputNumber :disabled="leaseModel.collectType != '3' || isEdit" v-model="leaseModel.fixedEarly" @on-change="canShowPlan" :max="31" :min="1" />
              </FormItem>
        ...
        <Row>
          <Col span="8">
            <FormItem class="item-label" label="开始收租日" prop="startCollectDate">
              <Select v-model="leaseModel.startCollectDate" :disabled="contractType == 0 || isEdit ">
    ...
    getDataDetail(roomId,leaseId){
      this.$ajax.get(
          ...
          res => {
              ...
              this.leaseModel.mortgageNum = Number(this.leaseModel.mortgageNum);
              //照片凭证
