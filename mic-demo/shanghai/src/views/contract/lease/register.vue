<template>
  <Card dis-hover>
       ...
       <FormItem class="item-label" label="合同模板" prop="templateId">
         <Row>
           ...
           <Col span="6" style="textAlign: center">
             <Button :to="{name:'contractTemplate/add'}" target="_blank" type="primary" ghost icon="md-add">点击创建新电子合同模板</Button>
           </Col>
         </Row>
       </FormItem>
     </Form> -->
            ...
            <Col span="8">
              <FormItem class="item-label" label="身份证正面">
                <ImgUploader
                  ...
                  @on-complete="this.frontImageComplete"
                  @on-remove="this.frontImgeRemove"
            ...
            <Col span="8">
              <FormItem class="item-label" label="身份证反面">
                <ImgUploader
                  ...
                  @on-complete="this.reversImageComplete"
                  @on-remove="this.reversImgeRemove"
                  :url="$api.FILE_UPLOAD_POST"
                  :maxUploads="1"
                  :uploadButton="'btn2'"
                />
    ...
    <Card dis-hover title="租约信息">
      <Form ref="leaseForm" label-position="right" :label-width="100" :model="leaseModel" :rules="rules">
        <Row>
          <Col span="8">
            <FormItem label="租约起止" class="item-label">
              <Row>
                  <Col span="11">
        ...
        <Row>
          <Col span="8">
            <FormItem class="item-label" label="房屋租金" prop="rent">
              <Input placeholder="请输入金额（元/月）" v-model="leaseModel.rent" @on-change="changeLeaseCost" number />
            </FormItem>
          </Col>
          <Col span="8">
            <FormItem class="item-label" label="房屋押金" prop="deposit">
              <Input placeholder="请输入金额（元）" v-model="leaseModel.deposit" @on-change="changeLeaseCost('deposit')" number />
            </FormItem>
          </Col>
        </Row>
        <Row>
          <Col span="8">
            <FormItem class="item-label" label="平方米月租金">
        ...
        <FormItem class="item-label" label="其他费用">
          <Row v-for="(item,index) in feeList" :key="index" style="marginBottom: 10px;">
            ...
            <Col span="3">
              <Button v-if="feeList.length >= 2" @click="deleteRow(index)" class="delete-row" shape="circle" icon="md-close"></Button>
            </Col>
          </Row>
        </FormItem>
        <FormItem>
          ...
          <RadioGroup @on-change="canShowPlan" vertical style="fontWeight:normal" v-model="leaseModel.collectType">
            <Radio label="0" style="margin-bottom:24px">
              ...
              <FormItem prop="beforeDay" style="display: inline-block;">
                <InputNumber :disabled="leaseModel.collectType != '0'" v-model="leaseModel.beforeDay" @on-change="canShowPlan" :max="99" :min="0"/>
              </FormItem>
              <span>天收租</span>
            </Radio>
            <Radio label="1" style="margin-bottom:24px">
              ...
              <InputNumber :disabled="leaseModel.collectType != '1'" v-model="leaseModel.fixedFirst" @on-change="canShowPlan" :max="31" :min="1" />
              </FormItem>
            ...
            <Radio label="2">
              ...
              <InputNumber :disabled="leaseModel.collectType != '2'" v-model="leaseModel.fixedEarly" @on-change="canShowPlan" :max="31" :min="1" />
              </FormItem>
              <span>日收租 (每期提前一个月)</span>
            </Radio>
          </RadioGroup>
        </FormItem>
       <!--  <FormItem class="item-label" label="房租使用">
          <Checkbox style="fontWeight: normal" v-model="leaseModel.isUseThird">该租约房租使用第三方金融</Checkbox>
          <Select :disabled="!leaseModel.isUseThird" v-model="leaseModel.Payway" style="width: 200px; fontWeight: normal;">
        ...
        <Row>
          ...
          <Alert show-icon>“常规收款模式” 不支持分期付款；<br>“分期模式” 支持租户信用卡一次性支付部分租金费用。前提需房东与第三方金融机构签订协议，并完成电子签申请</Alert>
            <FormItem class="item-label" label="收费方式" prop="-startCollectDate">
              <RadioGroup   v-model="leaseModel.landlordCharges1" @on-change2="landlordChange">
                  <Radio :disabled2="landLoadDisabled" label="1" >
                    常规收款模式
                  </Radio>
                  <Radio :disabled2="landLoadDisabled" label="0">
                    分期模式
                  </Radio>
              </RadioGroup>
            </FormItem>
         ...
         <Row>
          <Col span="12">
          <Alert show-icon>租客需一次性支付所需方案中的总计费用，手续费都有房东承担</Alert>
            <FormItem class="item-label" label="分期方式" prop="-startCollectDate" style="background-color: #f3f3f3;padding: 10px;">

              <RadioGroup style="vertical-align: center;"   v-model="leaseModel.landlordCharges1" @on-change2="landlordChange">
                  <Radio label="apple" >
                      <div style="display: inline-block;vertical-align: middle;padding:5px;background-color: #fff; ">
                        前<b>8</b>笔周期账单及押金总计<b>￥3400</b>，获得优惠<b>￥1000</b>。
                        <br>
                        总共需一次性支付<b>￥2400</b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;分6期还款，费率为7%
                      </div>
                  </Radio>
                  <Radio label="android" style="margin-top: 10px;">
                    <div style="display: inline-block;vertical-align: middle;padding:5px;background-color: #fff;">
                        前<b>8</b>笔周期账单及押金总计<b>￥3400</b>，获得优惠<b>￥1000</b>。
                        <br>
                        总共需一次性支付<b>￥2400</b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;分6期还款，费率为7%
        ...
        <Collapse simple style="border:none;" v-model="currentPanel">
          <Panel name="more">
            ...
            <div slot="content">
              ...
              <FormItem v-if="contractType == 0" class="item-label" label="公共家电家具">
                <Col span="6"
                  v-for="(item,index) in publicFurnitureList"
                  :key="item.name + index"
                  style="marginBottom: 10px;">
                  <FormItem>
                    <NumInput
                      ...
                      :min = "0"
                      :max = "99"
                      v-model="leaseModel.publicFurniture[item.name]"
                    />
                  </FormItem>
                </Col>
              </FormItem>
              <Tooltip :content="contractType == 0 ? '可上传房间物品照片等，最多20张' : '可上传合同、房间照片等，最多20张'">
                <FormItem class="item-label" label="照片凭证">
                  <!-- <ImgUploader
                    ...
                    @on-complete="this.proofCommplete"
                    @on-remove="this.proofRemove"
                    :url="$api.FILE_UPLOAD_POST"
                    :maxUploads="20"
                    :uploadButton="'btn4'"
                  /> -->
...
export default {
  ...
  data() {
    ...
    return {
      ...
      endOptions:{
        ...
        disabledDate :function(date) {
          //结束时间不小当前时间且不小于开始时间
            return  date.valueOf() < new Date() - 86400000 && date.valueOf() < this.leaseModel.startTime.valueOf();
        }.bind(this)
      },
      title:'租客登记',
      showAddress:false,
      ...
      leaseModel: {
        ...
        collectType: '0', // 收租方式 0-提前收租 1-每期的第一个月 2-每期提前一个月
        payNum: 1,
        beforeDay:10,
        fixedFirst:1,
        fixedEarly:1,
  ...
  methods: {
            ...
            let rentDay = Number(info.rentDay);
              switch (this.leaseModel.collectType) {
                ...
                case '2':
                  ...
                  break;
                default:
                  this.leaseModel.collectType = '0';
                  this.leaseModel.beforeDay = 10;
                  break;
              }

              if(info.constList && info.constList.length >=1){
                  //其他费用
                  this.feeList = [];//清空
                  info.costList.forEach(ele=>{
                    this.feeList.push({
                      ps:'',
    ...
    getDataDetail(roomId,leaseId){
      this.$ajax.get(
          ...
          res => {
              ...
              //照片凭证
              this.proofArray = this.leaseModel.proofArray;
        ...
        if(item && item.label){
          ...
          this.houseOptions.forEach((child)=>{
            ...
            */
              ...
              if(child.constList && child.constList.length >=1){
                  //其他费用
                  this.feeList = [];//清空
                  child.costList.forEach(ele=>{
                    this.feeList.push({
                      ps:'',
                      count:Number(ele.cost),
                      name:[ele.parName,ele.costNo],
                    ...
                    res => {
                        ...
                        this.houseOptions = res.data.content.map(item=>{
                          return {
                            ...
                            costList:item.roomCostList,//费用列表
    ...
    canShowPlan(planDate,type) {
      ...
      if ( !startTime || !endTime|| !payNum || !mortgageNum || !rent || !deposit ) {
        return;
      }
      //total 还根据第一期的结束时间改变
      ...
      for(let i = 0; i <= total+2; i++) {
        ...
        if (i !== 0) {
          ...
          } else if (collectType == '2' && fixedEarly) {
    ...
    // TODO  发请求(接口待确认)
      ...
      if(this.showAddress){
        ...
        }else if(query.type == 'room'){//房态图新增租约
            this.$ajax.post(
              this,
              this.$api.POST_NEW_LEASE,
              params,
    ...
    //返回列表
    toList(){
      this.$router.push({name:'contract/lease-list'})
    }
  }
}
</script>
