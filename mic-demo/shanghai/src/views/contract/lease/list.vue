  ...
  <Card dis-hover>
    <Tabs v-model="tabName" @on-click="changeTab" :animated="false">
      ...
      <div class="actions" slot="extra">
        <Button type="primary" icon="md-add" @click="routerToRegister(null)">租客登记</Button>
      </div>
    </Tabs>
  </Card>
</template>
        ...
        h('Button', {
          ...
          on: {
            click: (e) => {
              ...
              // this.$ajax.get(
              //   this,
              //   this.$api.CHECK_BILLING_STATUS.replace('{leaseId}',data.leaseId),
              //   {},
              //   (res)=>{
              //     if(res.data.content.haveBilling == '0'){
              //       this.$Modal.confirm({
              //         title: '提示',
              //         content: '<p>确定要作废该租约吗？</p>',
              //         onOk: () => {
