  ...
  <Card dis-hover>
      ...
      <Form v-if="moreCheck" ref="houseForm" label-position="right" :model="formModel" :label-width="100" :rules="rules" @submit.native.prevent>
        <Row>
          ...
          <Col offset="1" span="4">
                ...
                <Button type="primary" @click="selectHouseMore">新增</Button>
          </Col>
        </Row>
        <Row :gutter="20" style="padding:0px 30px;" v-if="moreCheck">
          <Col span="8" style="margin-bottom:10px;" v-if="checkHouseList.length>0" v-for="(item,index) in checkHouseList" :key="index">
            <Card style="border-radius:5px;padding:0px 10px;" class="list-card" dis-hover>
                <p slot="title" class="noWrap">
                  {{item.roomName}}
                    <div class="delete-icon" @click="changeLimit(index)"></div>
                </p>
    ...
    <Card dis-hover title="租约信息">
      <Form ref="leaseForm" label-position="right" :label-width="140" :model="leaseModel" :rules="rules">
        ...
        <Row>
          <Col span="8">
            <FormItem class="item-label" label="平方米月租金（元）" prop="squareMeterMonthlyRent">
              <Input v-model="leaseModel.squareMeterMonthlyRent"/>
            </FormItem>
          </Col>
          <Col span="8">
...
export default {
  ...
  data() {
    ...
    const checkMoneyS=(rule,value,callback)=>{
    ...
    return {
      ...
      rules: {
        ...
        squareMeterMonthlyRent:[
          {required: true,validator: checkMoneyS, trigger: 'change' }
        ],
        collectType: [
          {
          required: true,
  ...
  watch:{
    ...
    checkHouseList:function(val,oldVal){
      this.houseSpace=0;
      this.leaseModel.rent=0;
      this.leaseModel.deposit=0;
      this.leaseModel.squareMeterMonthlyRent=0;
      this.feeList=[
        {
          name: [],
          count: 0,
      ...
      this.leaseModel.roomConfigList=[];
      if(this.checkHouseList.length>0){
        let payNumFlag=true;
        let mortgageNumFlag=true;
        let collectTypeFlag=true;
        let rentDayFlag=true;
        let payNum=this.checkHouseList[0].payNum;
        let mortgageNum=this.checkHouseList[0].mortgageNum;
        let collectType=this.checkHouseList[0].collectType;
        let rentDay=this.checkHouseList[0].rentDay;
        this.checkHouseList.forEach((e)=>{
          if(e.payNum != payNum){
            payNumFlag=false;
          }
          if(e.mortgageNum != mortgageNum){
            mortgageNumFlag=false;
          }
          if(e.collectType != collectType){
            collectTypeFlag=false;
          }
          if(e.rentDay != rentDay){
            rentDayFlag=false;
          }
          this.houseSpace=Number(this.houseSpace)+Number(e.houseSpace);
          this.leaseModel.rent =Number(this.leaseModel.rent)+ Number(e.rent);
          this.leaseModel.deposit =Number(this.leaseModel.deposit)+ Number(e.deposit);
          this.leaseModel.squareMeterMonthlyRent = Number(e.squareMeterMonthlyRent);
          if(e.roomCostList && e.roomCostList.length >=1){
              if(this.feeList[0].name.length==0){
                this.feeList=[];
              }
              e.roomCostList.forEach((r)=>{
                this.feeList.unshift({
                  ps:'',
                  count:Number(r.cost),
                  name:[r.parName,r.costNo]
                })
              })
          }
          if(e.roomConfigList && e.roomConfigList.length >=1){
            e.roomConfigList.forEach((ele)=>{
              this.leaseModel.roomConfigList.push(ele.name);
            })
          }
        })
        if(payNumFlag){
          this.leaseModel.payNum=payNum;
        }else{
          this.leaseModel.payNum=Number(this.payNum);
        }
        if(mortgageNumFlag){
          this.leaseModel.mortgageNum=mortgageNum;
        }else{
          this.leaseModel.mortgageNum=Number(this.mortgageNum);
        }
        if(collectTypeFlag && rentDayFlag){
          this.leaseModel.collectType=collectType;
          this.leaseModel.rentDay=rentDay;
        }else{
          this.leaseModel.collectType=this.collectType;
          this.leaseModel.rentDay=this.rentDay;
        }
      switch (this.leaseModel.collectType) {
          case '1':
            this.leaseModel.beforeDay = Number(this.leaseModel.rentDay);
            break;
          case '2':
            this.leaseModel.fixedFirst= Number(this.leaseModel.rentDay);
            break;
          case '3':
            this.leaseModel.fixedEarly= Number(this.leaseModel.rentDay);
            break;
          default:
        ...
        if(this.formModel.houseId != '' && this.formModel.houseId !== undefined){
          let item;
          this.houseOptions.forEach((e)=>{
            if(e.value == this.formModel.houseId){
              item=e;
            }
          })
  ...
  methods: {
      ...
      this.$ajax.get(
        ...
        (res)=>{
          ...
          if(data.type){
            ...
            this.rentDay=data.date;
          }
          if(data.deposit != null){
            this.payNum=data.rent;
            this.mortgageNum=data.deposit;
          }
        }
      )
    },
    selectHouseMore(){
      if(this.formModel.houseId === undefined || this.formModel.houseId == '') return;
      let item;
      this.houseOptions.forEach((e)=>{
        if(e.value == this.formModel.houseId){
          item=e;
      ...
      let flag=true;
      this.checkHouseList.forEach((e)=>{
        if(e.roomId == this.formModel.houseId){
          flag=false;
        }
      })
      if(!flag) return;
          ...
          this.houseOptions.forEach((child)=>{
            ...
            if(child.value == item.value){
                this.checkHouseList.push({
                  ...
                  houseSpace:child.houseSpace,
                  rent:child.rent,
                  deposit:child.deposit,
                  squareMeterMonthlyRent:child.squareMeterMonthlyRent,
                  mortgageNum:child.mortgageNum,
                  payNum:child.payNum,
                  collectType:child.rentType,
                  rentDay:child.rentDay,
                  roomCostList:child.roomCostList,
                  roomConfigList:child.roomConfigList
                })
              // 多房源逻辑↑
    ...
    selectHouse(item){
      ...
      if(this.checkHouseList.length>0) return;
      if(this.formModel.houseId === undefined || this.formModel.houseId == '') return;
      this.houseOptions.forEach((e)=>{
          if(e.value==item.value){
            this.formModel.roomSpace=e.roomSpace;
    ...
    if(this.checkHouseList.length>1){
      ...
      if(this.checkHouseList.length == 1){
        params.roomNum=this.checkHouseList[0].roomName;
      }else{
...
<style>
  .list-card .ivu-card-body{
      padding:10px 0px;
  }
  .list-card .ivu-card-head{
    padding:6px 0px;
  }
</style>
