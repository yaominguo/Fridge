...
<Row :gutter="16">
  ...
  <Col :span="8">
    <Card dis-hover :bordered="false" :padding="0">
      <Carousel v-if="roomInfo.roomPicList && roomInfo.roomPicList.length > 0" autoplay :autoplay-speed="5000" loop>
        <CarouselItem v-for="item in roomInfo.roomPicList" :key="item.picPath">
            <img :src="item.picPath" class="carousel-item" />
        </CarouselItem>
      </Carousel>
      <div v-else class="carousel-item no-image">暂无房图</div>
      <div class="house-info">
        <p><span>房源编号：</span><span>{{roomInfo.id}}</span></p>
        <p><span>房源地址：</span><span>{{roomInfo.address || '暂无'}}</span></p>
        <!-- <p><span>房源概况：</span><span>概况的话</span></p> -->
        <p><span>房源面积：</span><span>{{roomInfo.roomSpace}}㎡</span></p>
        <!-- <p><span>房源类型：</span><span>{{roomInfo.houseKindName || '暂无'}}</span></p> -->
        <p><span>房型：</span><span>{{`${roomInfo.roomNum}室${roomInfo.parlourNum}厅${roomInfo.toiletNum}卫` }}</span></p>
        <p><span>房源标价：</span><span>{{$com.toMoney(roomInfo.rent)}}元/月</span></p>
      </div>
    </Card>
  </Col>
</Row>
